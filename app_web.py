...
if 'matriz_nombre' not in st.session_state:
    st.session_state.matriz_nombre = "MATRIZ_AQUAZ_OFICIAL.xlsx"

def guardar_en_disco():
    ruta = os.path.join(os.getcwd(), st.session_state.matriz_nombre)
    try:
        if os.path.exists(ruta):
            with pd.ExcelWriter(ruta, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
                for h, dataf in st.session_state.dfs.items():
                    if not dataf.empty:
                        dataf.to_excel(writer, sheet_name=h, index=False)
        else:
        ...
        st.error(f"Error al guardar físicamente: {e}")
...
# ==========================================
# 3. PANTALLA: CARGA DE DATOS (Intacta)
# ==========================================
if menu == "📥 Carga de Datos":
    ...
    if archivo_matriz:
        st.session_state.matriz_nombre = archivo_matriz.name
        ...
        st.success(f"✅ Matriz cargada en memoria.")
        st.markdown("---")
                ...
                if st.button("Procesar Ventas", use_container_width=True, type="primary"):
                    try:
                        ...
                        if lista_ventas:
                            ...
                            df_v = pd.DataFrame()
                            df_v['Fecha'] = pd.to_datetime(df_bruto.get('FECHA EMISION', pd.Series()), format='%d/%m/%Y', errors='coerce')
                            df_v['Empresa'] = empresa_activa 
                            df_v['Cliente'] = df_bruto.get('CLIENTE NOMBRE', pd.Series()).fillna("CLIENTE VARIOS")
                            df_v['Vendedor'] = df_bruto.get('ATENDIDO POR', pd.Series()).fillna("TIENDA")
                            df_v['Producto'] = df_bruto.get('PRODUCTO/SERVICIO', pd.Series()).fillna("SIN NOMBRE")
                            df_v['Cantidad'] = pd.to_numeric(df_bruto.get('CANTIDAD', pd.Series()), errors='coerce').fillna(0)
                            df_v['Precio_Venta'] = pd.to_numeric(df_bruto.get('PRECIO UNITARIO', pd.Series()), errors='coerce').fillna(0)
                            df_v['Costo_Unitario'] = pd.to_numeric(df_bruto.get('COSTO UNITARIO', pd.Series()), errors='coerce').fillna(0)
                            df_v['Descuento'] = pd.to_numeric(df_bruto.get('DESCUENTO', pd.Series()), errors='coerce').fillna(0)
                            df_v = df_v[df_v['Cantidad'] > 0]
                            
                            st.session_state.dfs['Ventas'] = pd.concat([st.session_state.dfs['Ventas'], df_v], ignore_index=True)
                            guardar_en_disco()
                            st.success(f"¡{len(df_v)} ventas inyectadas!")
                    except Exception as e:
            ...
            if archivo_prod:
                if st.button("Procesar Producción", use_container_width=True, type="primary"):
                    try:
                        ...
                        df_bruto.columns = df_bruto.columns.str.strip()
                        df_p = pd.DataFrame()
                        df_p['Fecha'] = pd.to_datetime(df_bruto.get('Fecha', pd.Series()), errors='coerce')
                        df_p['Empresa'] = empresa_activa
                        df_p['Lote'] = df_bruto.get('Número de Lote (o de Orden)', pd.Series()).fillna("S/L")
                        df_p['Producto'] = df_bruto.get('Producto Fabricado', pd.Series()).fillna("SIN NOMBRE")
                        df_p['Cantidad_Producida'] = pd.to_numeric(df_bruto.get('Cantidad Producida', pd.Series()), errors='coerce').fillna(0)
                        df_p['Costo_Materia_Prima'] = pd.to_numeric(df_bruto.get('Costo de Materia Prima', pd.Series()), errors='coerce').fillna(0)
                        df_p['Merma_Soles'] = pd.to_numeric(df_bruto.get('Merma', pd.Series()), errors='coerce').fillna(0)
                        df_p['Operario'] = df_bruto.get('Operario / Responsable', pd.Series()).fillna("No especificado")
                        
                        st.session_state.dfs['Produccion'] = pd.concat([st.session_state.dfs['Produccion'], df_p], ignore_index=True)
                        guardar_en_disco()
                        st.success(f"¡{len(df_p)} registros inyectados!")
                    except Exception as e:
...
# ==========================================
    ...
    if df is None or df.empty:
        ...
        st.markdown("---")
        # NUEVO: Churn Real conectado a Facel
...
# ==========================================
# 6. PANTALLA: FINANZAS Y COSTOS (Prorrateo y Semáforo)
# ==========================================
    ...
    # NUEVO: Semáforo de Rentabilidad real leyendo Facel
    with colB:
